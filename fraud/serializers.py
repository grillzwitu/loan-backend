...
class FlaggedLoanSerializer(serializers.ModelSerializer):
    ...
    """
    fraud_flags: serializers.ListSerializer = FraudFlagSerializer(many=True, read_only=True)

    class Meta:
        model = LoanApplication
        fields: tuple[str, ...] = ("id", "user", "amount", "status", "created_at", "updated_at", "fraud_flags")
        read_only_fields: tuple[str, ...] = ("id", "user", "amount", "status", "created_at", "updated_at", "fraud_flags")
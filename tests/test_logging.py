...
def test_run_fraud_checks_logging(caplog: LogCaptureFixture) -> None:
    """
    run_fraud_checks should log a WARNING when a loan is flagged for high amount.
    """
    caplog.set_level(logging.WARNING, logger="fraud.services")
    user = User.objects.create_user(username="user_log", email="log@example.com", password="password")
...
def test_withdraw_endpoint_logging(caplog: LogCaptureFixture) -> None:
    ...
    # Obtain tokens
    login_resp = client.post(reverse("token_obtain_pair"), {"username": user.username, "password": "password"}, format="json")
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {login_resp.data['access']}")
    # Create a new loan
    create_resp = client.post(reverse("loan-list-create"), {"amount": "1000.00"}, format="json")

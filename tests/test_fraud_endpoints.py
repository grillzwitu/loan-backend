...
These tests ensure that flagged loans are recorded via the FraudFlag model,
that admin users can list flagged loans with nested fraud reasons,
and that regular users are forbidden from accessing this endpoint.
"""
import pytest
...
def test_flagged_loans_endpoint_admin(admin_client: APIClient, user: User) -> None:
    """
    ...
    loan: LoanApplication = LoanApplication.objects.create(user=user, amount=6000000)
    ...
    flagged_data: Any = next(item for item in result_items if item["id"] == loan.id)
    assert flagged_data["id"] == loan.id
...
@pytest.mark.django_db
def test_flagged_loans_endpoint_forbidden_regular_user(auth_client: APIClient, user: User) -> None:
    """
